...
#    geom_vline(xintercept=as.POSIXct(as.Date(c("2020-01-23","2020-02-19", "2020-11-05","2020-11-09"))) , #size=0.5, alpha = 0.5, style="dash")+
  ...
  labs(title = "Stock market reaction to the Covid19 Pandemic",
       subtitle = "Performance of major indeces in 2020")+
  
  
  annotate("text", x = as.POSIXct(as.Date(c("2020-01-10","2020-01-10","2020-02-14","2020-05-09","2020-10-01", "2020-11-21","2020-11-23"))), 
           y = c(-0.22,-0.24, -0.27,-0.35,-0.35,-0.25,-0.28), 
           label = c("First Lockdown", "China:Wuhan","1st Wave Hits Europe","USA Lockdown","2nd Wave Hits Europe", "Vaccine","Announces") , 
           color="black", size=5 , angle=0, fontface="bold")+
  annotate("segment", x = as.POSIXct(as.Date(c("2020-01-23","2020-02-19","2020-10-26","2020-11-09" ))), 
           xend = as.POSIXct(as.Date(c("2020-01-23","2020-02-19","2020-10-26","2020-11-09" ))), 
           y = c(-0.2,-0.25,-0.38,-0.32), 
           yend = c(-0.1,-0.15,-0.28,-0.22), colour = "darkgray", size=2, alpha=1, arrow=arrow())
...
pivot_longer( cols = c(`sse`, `ftse100`, `s_p500`, `euronext`, `bse_sensex` , `topix`), names_to = 'market' , values_to = 'price')%>%
  ...
  summarise(first_val = first(price)  , last_val = last(price)) %>% 
...
map_covid  %>%
  ...
  theme(legend.direction = "horizontal",
        ...
        panel.grid.major.y = element_blank() , panel.grid.minor.y = element_blank()) +
  labs(title = "",
       subtitle = "Total number of Covid Cases per country ")
