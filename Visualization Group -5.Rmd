...
early_covid %>% 
  pivot_longer( cols = c(2:9), names_to = 'market' , values_to = 'pct_change') %>% 
  filter(is.na(pct_change) == FALSE) %>% 
  ggplot() +
  geom_line(aes(x = dates, y=pct_change, color = market) , size = .5 , alpha = 0.5) + 
  geom_hline(yintercept = 0 , size = 2 ) + 
  scale_y_continuous(labels =  scales::percent) + 
  theme_minimal() + 
  theme(panel.grid.major.x =  element_blank() , 
...
plot_df1 %>%
  ...
  ggplot() + 
  geom_col(aes(x = as.factor(year) , y = perc_drop , fill = market) , position = 'dodge' , alpha = 0.6, width = .5) + 
  theme_minimal() +
...
draw_plot <- originalData_process %>% 
  select(dates, s_p500 ) %>%
  filter(is.na(s_p500) == FALSE) %>% 
  mutate(month = month(dates))

draw_plot %>% 
    ggplot() +
    geom_line(aes(x = dates , y = s_p500))
...
draw_plot %>% 
  filter(year %in% c(2000, 2001 , 2003 , 2008 ,2009 , 2018 ,2020)) %>%
  group_by(year) %>% 
  mutate(max = max(s_p500))
  
  

findpeaks(draw_plot$s_p500 ,minpeakdistance = 150 )


  
ggplot(draw_plot) + 
  geom_line(aes(x = dates, y = s_p500) , alpha = 0.6) + 
  theme_minimal() +
  scale_fill_brewer(palette = 'Dark2') + 
  theme(panel.grid.minor = element_blank() ,
        panel.grid.major.x  = element_blank(), 
...
plot_sp_2020 <- originalData_process %>%
  mutate(year = year(dates) , month = month(dates)) %>%
select("dates","year","month","s_p500") %>%  
  filter(year == 2020) %>%
  ggplot(aes(y = s_p500, x = dates)) +
  geom_line()
plot_sp_2020
...
# s_p_only <- indeces %>%
#   select("Dates", "S&P 500 (^SPX) - Index Value")
# 
# s_p_only %>%
#   mutate(year = year(Dates) , month = month(Dates))

indeces_recovery <- indeces %>%
  mutate(year = year(Dates) , month = month(Dates)) %>%
  filter(year == 2020) 
indeces_recovery

indeces_recovery_sp <- indeces %>%
  mutate(year = year(Dates) , month = month(Dates)) %>%
  filter(year == 2020) %>%
  select("Dates", "S&P 500 (^SPX) - Index Value")
indeces_recovery_sp

plot_V_2020 <-ggplot(indeces_recovery_sp, aes(y = "S&P 500 (^SPX) - Index Value", x = Dates)) +
  geom_line()
plot_V_2020
